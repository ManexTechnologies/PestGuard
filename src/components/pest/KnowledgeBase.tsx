...
import { Search, Filter, BookOpen, ChevronDown, ChevronUp, Shield, Leaf, Bug, FlaskConical, Sprout, AlertTriangle, X, Info } from 'lucide-react';

import { KNOWLEDGE_BASE, CROP_TYPES, SEVERITY_COLORS, SEVERITY_DOT_COLORS, type PestInfo, type Treatment } from '@/data/pestData';
...
const KnowledgeBase: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Pest Knowledge Base</h2>
        <p className="text-gray-500 mt-1">Browse {KNOWLEDGE_BASE.length} common Zimbabwean crop pests with treatment guides</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search pests by name, crop, or description..."
        ...
        <p className="text-xs text-gray-500 mt-2">{filteredPests.length} pest{filteredPests.length !== 1 ? 's' : ''} found</p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedPest(null)}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              <div>
                <h3 className="text-xl font-bold text-gray-900">{selectedPest.name}</h3>
                <p className="text-sm text-gray-500 italic">{selectedPest.scientificName}</p>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="flex flex-wrap gap-2">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${SEVERITY_COLORS[selectedPest.severity]}`}>
                  <span className={`w-2 h-2 rounded-full ${SEVERITY_DOT_COLORS[selectedPest.severity]}`} />
                  {selectedPest.severity} severity
                </span>
                <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-medium">{selectedPest.type}</span>
                <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-medium">{selectedPest.season}</span>
              </div>
              ...
              <div>
                <h4 className="font-semibold text-gray-900 mb-2">Affected Crops</h4>
                <div className="flex flex-wrap gap-2">
                  {selectedPest.cropAffected.map(crop => (
                    <span key={crop} className="px-3 py-1 bg-green-50 text-green-700 rounded-full text-xs font-medium border border-green-200">{crop}</span>
                  ))}
                </div>
              </div>

              <div>
                <h4 className="font-semibold text-gray-900 mb-2">Damage Symptoms</h4>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full border capitalize ${treatmentTypeBadge(treatment.type)}`}>
                              {treatment.type}
                            </span>
                            <span className={`text-xs px-2 py-0.5 rounded-full ${
                              treatment.effectiveness === 'high' ? 'bg-green-100 text-green-700' :
                              treatment.effectiveness === 'medium' ? 'bg-yellow-100 text-yellow-700' :
            ...
            >
              <div className="h-40 bg-gray-200 relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${SEVERITY_COLORS[pest.severity]}`}>
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <div className="flex flex-wrap gap-1">
                    {pest.cropAffected.slice(0, 3).map(crop => (
                      <span key={crop} className="px-2 py-0.5 bg-white/80 backdrop-blur-sm text-gray-700 rounded text-xs font-medium">{crop}</span>
                    ))}
                    {pest.cropAffected.length > 3 && (
                      <span className="px-2 py-0.5 bg-white/80 backdrop-blur-sm text-gray-500 rounded text-xs">+{pest.cropAffected.length - 3}</span>
                    )}
                  </div>
                </div>
              </div>
              <div className="p-4">
                <h3 className="font-semibold text-gray-900 group-hover:text-green-700 transition-colors">{pest.name}</h3>
                <p className="text-xs text-gray-500 italic mb-2">{pest.scientificName}</p>
                <p className="text-sm text-gray-600 line-clamp-2 mb-3">{pest.description}</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">{pest.type}</span>
                  <span className="text-xs text-green-600 font-medium flex items-center gap-1">
                    {pest.treatments.length} treatments <Info className="w-3 h-3" />
                  </span>
                </div>
              </div>
            </div>
